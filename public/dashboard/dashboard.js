...
function renderSection() {
  switch (state.section) {
    ...
    case 'ai': return renderToggleGroup('AI settings', [['ai.enabled','AI enabled'],['ai.memoryEnabled','Memory enabled'],['ai.naturalInvocation','Natural invocation']], [['ai.dailyLimit','Daily AI limit','number'],['ai.memoryMaxMessages','Memory messages','number'],['ai.commandPrefix','Command prefix','text']]) + renderPersona(state.persona);
...
function renderDashboard() {
  return '<div class="layout"><aside class="sidebar"><div class="brand"><div class="logo"><img src="/logo.svg" alt="OmniBot"/></div><div>OmniBot</div></div><nav class="nav"><div class="nav-label">General</div>' +
    ...
    '</nav><button class="btn ghost" id="btnChangeServer">Change server</button><button class="btn ghost" id="btnLogout">Log out</button></aside><main class="main"><div class="topbar"><h1 style="margin:0">' + escapeHtml(state.section) + '</h1><span class="pill">' + escapeHtml(state.guild.name) + '</span></div>' + renderSection() + '</main></div>';
}
function bind() {
  ...
  root.querySelectorAll('[data-section]').forEach(function (btn) { btn.addEventListener('click', function () { state.section = btn.getAttribute('data-section'); render(); }); });
